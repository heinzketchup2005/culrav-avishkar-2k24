...
const ForgetPassword = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setisSending(true);
    apiClient.post(`/api/auth/v1/sendForgotPasswordToken`, { email }).then(result => console.log(result))
  ...
  <div className="flex flex-col items-center justify-center w-full max-w-md p-6 bg-[#2D2D2D]  mx-4 sm:mx-0">
        <h1 className="text-2xl text-[#FFFAF0] font-bold font-bionix mb-6 text-center">Forget Password</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          />
          {sending?<Loader2 className="w-full bg-[#F54E25] hover:bg-orange-500 text-white py-3  font-semibold animate-spin color-white" />:
            <button
            type="submit"
        ...
        <div className="flex w-full justify-between mt-4 px-2">
          <Link to="/home" className="text-[#FFFAF0] font-sftext hover:underline">Home</Link>
          <Link to="/login" className="text-[#FFFAF0] font-sftext hover:underline">Login</Link>
        </div>
      </div>
    </div>
  );
