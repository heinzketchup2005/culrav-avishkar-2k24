...
function Register() {
  ...
  const validatePasswordMatch = (value) => value === watch("password") || "Passwords do not match";
  ...
  <div className="flex flex-col items-center justify-center w-full max-w-md p-6 bg-[#2D2D2D]  mx-4 sm:mx-0">
    ...
    <form onSubmit={handleSubmit(create)} className="space-y-5  font-sftext w-full">
      <Input
        placeholder="Full name"
        {...register("name", { required: "Full name is required" })}
      />
      {errors.name && <p className="text-[#F54E25]">{errors.name.message}</p>}
      ...
      <Input
        ...
        {...register("email", {
          ...
          validate: {
            matchPattern: (value) =>
              /^[a-zA-Z0-9]+(\.[0-9]+)*@mnnit\.ac\.in$/.test(value) ||
              "Email must be a valid MNNIT email address",
          },
        })}
      />
      {errors.email && <p className="text-[#F54E25]">{errors.email.message}</p>}
    ...
    >
      ...
      {errors.password && <p className="text-[#F54E25]">{errors.password.message}</p>}
    ...
    >
      ...
      {errors.confirmPassword && (
        <p className="text-[#F54E25]">{errors.confirmPassword.message}</p>
          ...
          {submiting?<Loader2 className="animate-spin justify-center items-center"/>:
            <Button type="submit" className="w-full font-sftext bg-orange-600 hover:bg-orange-500 text-[#FFFAF0] py-3  font-semibold">
              Register
            </Button>}
    </form>
